...
- ## Why this matters for the [[wiki/projects/growing-computation]] thread
	- **Provides the bridge from soft training to hard substrate.** SODC, growing-networks, EngramNCA all train continuous rules. DiffLogic CA's contribution is the *compilation step* — that continuous rule can be reduced to a discrete circuit of bounded size, deployable on physical logic-gate substrates like [[wiki/concepts/fpga]]. Without this bridge, "self-organising on hardware" remains a software metaphor.
...
- ## The SODC flip
	- SODC ([[wiki/research/self-organising-digital-circuits]]) puts the NCA *outside* the boolean circuit — the NCA is the optimizer, the circuit is the substrate being optimised.
	- DiffLogic CA inverts this — the NCA *is* a boolean circuit. The substrate runs the NCA natively.
	- **Together they bracket the project.** Growing-computation is the synthesis: the regulatory NCA *runs* on the boolean substrate (DiffLogic-style) AND it orchestrates the substrate's role assignments (SODC-style, but for hardware regions rather than gate-types). ^[claude-synth]
