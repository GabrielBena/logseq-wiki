...
category:: reference
type:: external-ref
tags:: neuromorphic, plasticity, homeostasis, excitation-inhibition, dynap-se2, device-mismatch
...
pmid:: 40595529
summary:: Indiveri/INI cross-homeostatic E/I plasticity autonomously tunes a FIXED spiking net on DYNAP-SE2 to stable inhibition-stabilized dynamics despite analog mismatch — dynamics, not function.
confidence:: 0.70
lifecycle:: draft
...
- **Stable recurrent dynamics in heterogeneous neuromorphic computing systems using excitatory and inhibitory plasticity** — Maryada, Soldado-Magraner, Sorbaro, Laje, Buonomano, **Indiveri** (2025), *Nature Communications* (DOI 10.1038/s41467-025-60697-2; bioRxiv 2024). **From Indiveri's group (INI Zürich).** The closest prior art to a *learned self-organising compiler* on analog neuromorphic hardware — characterised here precisely to mark the contrast (see below). **Full-ingested 2026-07-02** from the OA PDF. ^[author]
- ## What it is
...
- ## Relation & contrast to a self-organising compiler
	- **Shared spirit:** a local, biologically-inspired plasticity rule autonomously configures analog neuromorphic hardware into a useful regime **despite device mismatch** — self-calibration by a local rule, no per-chip manual tuning. The same *ethos* as a self-organising compiler; the contrast is below. ^[extracted] ^[inferred]
	...
	- **Fixed-net + hand-crafted, not compile/grow + meta-learned:** topology is fixed random; only **4 global weight scalars** are tuned — no structure growth, no recruitment, no continual/task learning (one-shot post-fab *calibration*; memories are *manually implanted* by weight-masking). The rule is a **hand-derived fixed homeostatic update** with hand-set targets — *not* a **meta-learned** compiler; exactly the axis a self-organising compiler ([[wiki/projects/self-organising-mosaic-compiler]]) differentiates on. ^[extracted] ^[inferred]
- ## Connections to Wiki
	- [[wiki/projects/self-organising-mosaic-compiler]] — the contrast case: a self-organising compiler owns placement/routing/recruitment; this tunes a fixed net's dynamics.
