...
- **Memory devices and applications for in-memory computing** — Sebastian, Le Gallo, Khaddam-Aljameh, Eleftheriou (2020), *Nature Nanotechnology* 15:529–544 (Review). **IBM Research – Zürich.** A broad review grounding the "in-memory computing is the route past the memory wall, but device non-idealities are the barrier" framing. ^[author]
- ## Abstract (extracted)
...
- ## Key ideas ^[inferred]
	...
	- The Review's Challenges section names **device non-idealities + scaling characteristics** as the barrier to next-generation in-memory computing (noise, drift, and write variability fundamentally limit it; on-chip training accuracy stays open). ^[inferred]
- ## Connections to Wiki
...
- ## Open Questions / To Read
	- Abstract-mode: the non-ideality taxonomy + on-chip-training-accuracy limits live in the paper body. Upgrade to full with `/paper-ingest full` if exact figures are needed. ^[inferred]
