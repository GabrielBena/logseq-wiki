...
- ## TL;DR
	...
	- A single simple cell-division rule reproduces the observed patterns: `c_i = c_{parent} + δ_i, δ_i ~ N(0, σ)` plus an H-tree fractal placement of daughters near parents. A striking biological **parallel** to the chemo-affinity birth + smooth-`φ` machinery sketched in [[wiki/concepts/developmental-latent-space]]: eigengenes look a lot like engrams; lineage inheritance resembles `g_daughter = g_parent + ε`. ^[claude-synth]
...
- ## Connections to Gabriel's Research
	- ### Direct biological grounding for [[wiki/concepts/developmental-latent-space]]
		...
		- Kerstjens et al. is the **biological version of the same architecture**:
			...
			- *Manifold inheritance ↔ lineage.* `g_d = g_parent + ε` has the same *form* as `c_i = c_{i/2} + δ_i, δ_i ~ N(0, σ)` (Equation 5) — a clean formal echo, modulo what the symbols mean.
		...
		- This **strengthens** the developmental-latent-space concept (confidence nudged up) once the connection is wired in. Five of its claims (engram ≈ type code; smooth `φ`; lineage-style inheritance; degeneracy of sparse subsets; multi-scale clustering) find a degree of *empirical resonance* in this Neuron/2026 paper — support for the analogy, not proof of identity. ^[claude-synth]
